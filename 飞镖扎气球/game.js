...
(function () {
  ...
  function bindInput() {
    if (ttApi && ttApi.onTouchStart) {
      ttApi.onTouchStart(function (event) {
        ...
        if (t) beginDrag(toGamePoint(touchX(t), touchY(t)));
      });
      ttApi.onTouchMove(function (event) {
        var t = event.touches && event.touches[0];
        if (t) moveDrag(toGamePoint(touchX(t), touchY(t)));
      });
      ttApi.onTouchEnd(function (event) {
        var t = event.changedTouches && event.changedTouches[0];
        endDrag(t ? toGamePoint(touchX(t), touchY(t)) : dragCurrent);
      });
      return;
    }
  ...
  function touchX(touch) {
    return touch.clientX != null ? touch.clientX : touch.x;
  }

  function touchY(touch) {
    return touch.clientY != null ? touch.clientY : touch.y;
  }
  ...
  function requestFrame(fn) {
    if (canvas.requestAnimationFrame) canvas.requestAnimationFrame(fn);
    else if (typeof requestAnimationFrame !== "undefined") requestAnimationFrame(fn);
