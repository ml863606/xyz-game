...
def detect_frameworks(package: dict[str, Any] | None, files: list[Path], project: Path) -> list[str]:
    ...
    if (project / "next.config.js").exists() or (project / "next.config.ts").exists() or any(p.name in {"next.config.js", "next.config.ts"} for p in files):
        found.add("Next.js")
    if (project / "project.godot").exists() or any(p.suffix.lower() in {".gd", ".tscn"} for p in files):
        found.add("Godot")
    return sorted(found)
...
def classify(path: Path, project: Path) -> str:
    ...
    name = path.name
    if name in ENTRY_NAMES or r in {"src/app/page.tsx", "src/app/layout.tsx", "app/page.tsx", "app/layout.tsx", "scenes/main.tscn"} or r.endswith("/src/app/page.tsx") or r.endswith("/src/app/layout.tsx"):
        return "entry"
    if path.suffix.lower() == ".tscn" or "/scenes/" in r:
        return "page"
