...
def render_manual_canonical(
    ...
    core_value_text = clean_field(core_value, "软件可以帮助用户完成主要操作并及时查看操作结果。")
    flow = operation_flow
    ...
    lines.extend(
        [
            ...
            "",
            f"{software_name} {version}适用于{industry_text}场景。用户启动软件后，可以在竖屏主界面查看当前回合状态，完成瞄准、发射、命中目标、查看分数和继续挑战等操作。",
            "",
            f"日常使用时，{user_text}可以按照画面提示查看目标分数、剩余时间、任务、风向和能量状态，并通过鼠标或触摸拖拽完成飞镖发射。{core_value_text}",
            "",
        ]
    )
    if positioning_text:
    ...
    lines.extend(
        [
            "",
            f"请确保实际运行环境满足以上要求，以保证{software_name}能够正常启动、显示竖屏画面、响应拖拽输入并展示游戏结果。若运行设备或打包方式与本表不同，应以实际确认的申请表环境字段为准。",
            "",
        ]
    )
