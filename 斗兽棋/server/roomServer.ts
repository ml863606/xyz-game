...
export function startRoomServer(port = PORT): Promise<void> {
  return new Promise((resolveStart, reject) => {
    ...
    const onListening = (): void => {
      ...
      console.log(`斗兽棋房间服务已启动：ws://localhost:${port}`);
      resolveStart();
    };
...
function flipPiece(socket: WebSocket, message: Extract<ClientMessage, { type: 'flip_piece' }>): void {
  const room = rooms.get(message.roomId);
  const player = room?.players.find((item) => item.id === message.playerId);
  if (!room || !player?.side) {
    send(socket, { type: 'error', code: 'missing_room', message: '房间或玩家不存在' });
  ...
  const current = room.game.snapshot();
  if (message.moveNumber !== current.moveNumber) {
    send(socket, { type: 'move_rejected', reason: '棋局状态已更新，请同步后再翻', state: current });
    return;
  }

  const result = room.game.flip(message.position, player.side, player.id);
  if (!result.ok) {
    send(socket, { type: 'move_rejected', reason: result.reason ?? '无法翻 牌', state: result.state });
    return;
  }

  if (result.assignedSide) {
    assignRoomSides(room, player.id, result.assignedSide);
  }
...
function assignRoomSides(room: Room, playerId: string, assignedSide: PlayerSide): void {
  const player = room.players.find((item) => item.id === playerId);
  ...
  if (opponent) {
    opponent.side = assignedSide === 'red' ? 'blue' : 'red';
  }
}
