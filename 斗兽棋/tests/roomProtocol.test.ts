...
test('room server supports create, join, ready, flip rejection and reconnect', async () => {
  ...
  try {
    ...
    assert.match(rejected.reason, /还没轮到你/);
    ...
    const flipped = await red.next('room_state', (message) => message.room.state.moveNumber === 1);
    const flippedPiece = flipped.room.state.pieces[flipped.room.state.lastMove?.pieceId ?? ''];
    assert.equal(flipped.room.state.turn, flippedPiece.side === 'red' ? 'blue' : 'red');
    assert.equal(Boolean(flippedPiece.revealed), true);

    red.close();
    const replacement = await connect();
    replacement.sendJson({ type: 'reconnect', roomId, playerId: 'red-player', nickname: '红方回来了' });
    const rejoined = await replacement.next('room_joined');
    assert.equal(rejoined.room.state.moveNumber, 1);
    assert.equal(rejoined.side, flipped.room.players.find((player) => player.id === 'red-player')?.side);
